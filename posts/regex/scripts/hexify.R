# https://github.com/djnavarro/hexify/tree/main
#'
...
hexify <- function(from,
  ...
  image <- sticker_border(image,
                          opacity = border_opacity,
                          colour = border_colour)

  image <- sticker_annotate(image,
...
sticker_crop <- function (image, crop) {
  ...
  if (width > height) {
    ...
    geom <- magick::geometry_area(height,
                                  height,
                                  x_off = (width - height) / 2,
                                  y_off = 0)
  ...
  if (height > width) {
    ...
    geom <- magick::geometry_area(width,
                                  width,
                                  x_off = 0,
                                  y_off = (height - width) / 2)
...
sticker_rescale <- function(image,
                            hex_width) {
...
sticker_border <- function(image,
  ...
  base_hex_img <- system.file("extdata",
                              "base_hex.png",
                              package = "hexify")
