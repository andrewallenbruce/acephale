...
random_hcpcs_vec <- \(n = 10) {
  ...
  c(
    sample(c(LETTERS[sf_grepl(LETTERS, "[^DINOW-Z]", nthreads = 4L)], 0:9), size = nn),
...
#| echo: false
color_vctr(
  as.character(c(9, 0, 4, 6, 0)),
  text_color = c(NA, "blue",      "green",  "black", NA),
  style      = c(NA, "underline", "italic", "bold",  NA),
  background = c(NA, NA,          NA,       "white",  NA))
```
...
view <- \(x) {
  ...
  tree <- unname(purrr::map_vec(x, p, sep = ' ')) |> purrr::map_vec(cutoff)
