...
remove_redundant <- function(x, verbose = TRUE) {
  ...
  if (verbose) {
    # cli::cat_boxx(label = paste0("Group 3", cli::symbol$record), align = "left", padding = c(0, 1, 0, 1))
...
# collapse::get_elem(x, "r", regex = TRUE)
# collapse::get_elem(x, "o", regex = TRUE)
# collapse::get_elem(x, "f", regex = TRUE)
...
g3 <- function(x, verbose = TRUE) {
  ...
  if (verbose) {
    cli::cat_line(label = paste0(cli::symbol$record, " Group 3"), col = "red")
    # cat("** Group 3:", sep = "\n")
    cat(view(out), sep = "\n")  
    return(invisible(out))
...
p1 <- function(x) {
  ...
  postfix <- ifelse(stringfish::sf_grepl(re, "\\[?[0-9]{1}", nthreads = 4L), "[A-Z0-9]", "[0-9]")
