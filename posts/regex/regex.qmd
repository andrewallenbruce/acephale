...
# Length 4 Start with a Letter
if (any(sf_detect(x$x4, "^[A-Z]"))) {
  
  # Isolate codes starting with letter
  x4s  <- sf_extract(x$x4, "^[A-Z]")
  
  # Isolate letters
  sf_sub(x4s, 1, 1)
  # Isolate codes, sans letters
  sf_remove(x4s, "^[A-Z]")
  
  # Remove letter codes from original vector
  x$x4[!sf_detect(x$x4, "^[A-Z]")]
}
```
...
# Length 5 End with Letter
# if (any(sf_detect(x$x5, "[A-Z]$"))) {

x$x5

# Isolate codes starting with letter
  x5e <- sf_extract(x$x5, "[A-Z]$")
  
  glue::glue("{take_at(x5e, 5)}{sf_remove(x5e, '[A-Z]$')}")

  # Remove letter codes from original vector
  x$x5[!sf_detect(x$x5, "^[A-Z]")]
# }
```
...
#| message: true
#| echo: true
split_at <- \(x, i, z) collapse::rsplit(x, sf_sub(x, i, z), use.names = FALSE)

sp1 <- \(x) split_at(x, 1, 1)
