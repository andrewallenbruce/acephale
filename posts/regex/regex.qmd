...
len <- \(rx) cheapr::vector_length(vctrs::vec_slice(hcpcs, stringfish::sf_grepl(hcpcs, rx)))
...
```{r}
#| label: random_hcpcs_vec2
random_hcpcs_vec2 <- \(n = 10) {
  
  ltrs <- LETTERS[stringfish::sf_grepl(LETTERS, "[^DINOW-Z]")]
...
detect_az <- \(x) x[sf$grepl(x, "[A-Z]{1}")]
```
