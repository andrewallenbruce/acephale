...
hcpcs_codes <- stringfish::convert_to_sf(
  collapse::funique(
    northstar::search_descriptions()$hcpcs_code)
  )
```

# General HCPCS Regexes

18,903 total HCPCS codes

```{r}
#| label: hcpcs_all
hcpcs_all_regex <- "(^[A-CEGHJ-MP-V0-9][0-9]{3}[AFMTU0-9]$)"
...
```{r}
#| label: hcpcs_lvlI
hcpcs_lvlI_regex <- "(^[0-9]{4}[AFMTU0-9]$)"
...
```{r}
#| label: hcpcs_lvlII
hcpcs_lvlII_regex <- "(^[A-CEGHJ-MP-V][0-9]{4}$)"
...
g3 <- function(x) {
  ...
  out <- vctrs::vec_c(
    ...
    if (empty(last)) NULL else vctrs::vec_chop(last$code, sizes = vctrs::vec_run_sizes(last$a1)),
    if (empty(rest)) NULL else vctrs::vec_chop(rest$code, sizes = vctrs::vec_run_sizes(rest$a1))
  )
...
vrg <- function(x, n) {
  ...
  x <- ifelse(
    n == 0, 
    glue::glue("(^{x}$)"),
    glue::glue("(^<<x>><<postfix>>{<<n>>}$)", .open = "<<", .close = ">>")
    )
