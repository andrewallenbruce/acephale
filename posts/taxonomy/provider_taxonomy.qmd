...
taxonomy <- xwalk |> 
  ...
  # dplyr::mutate(
  #   dlim = dplyr::case_when(
  #     codex::sf_detect(taxonomy_type, "/") ~ "fslash", 
  #     codex::sf_ndetect(taxonomy_type, "/") ~ "comma"),
  #   n = dplyr::case_when(
  #     codex::sf_detect(taxonomy_type, "/") ~ stringr::str_count(taxonomy_type, "/"), 
  #     codex::sf_ndetect(taxonomy_type, "/") ~ stringr::str_count(taxonomy_type, ","))) |> 
...
updates <- base |>
  ...
  tidyr::separate_longer_delim(cols = updates, delim = stringr::fixed("; ")) |> 
  dplyr::mutate(updates = stringr::str_remove_all(updates, "(^\\[)|(\\]$)"),
                first = stringr::str_extract_all(updates, ", \\d{1,2}")) |>
  tidyr::unnest(first, keep_empty = TRUE) |>
  dplyr::mutate(first = str_remove(first, ", "))

updates <- vctrs::vec_rbind(
  updates |> 
    dplyr::mutate(multi = str_count(updates, ", \\d{1}/\\d{1}/\\d{4}")) |>
