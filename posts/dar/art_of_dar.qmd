...
| **Aging of Accounts**
|    An **account** is a billable episode of care. It begins to **age** once 
|    it is billed to the **guarantor**. These *outstanding* accounts are 
|    monitored in 30-day blocks or **bins** ($[0, 30)$, $[31, 60)$, $[61, 90)$, 
|    $[91, 120)$, and $[121, \infty)$) by the number of accounts, the amount
|    owed by each bin and the responsible parties. The older the account, 
|    the less likely it will be paid.
...
| $t = \text{Time Period}$ 
|      The subscript $t$ denotes the time period being measured.
| $n_{days} = \text{Days in Period}$
|      The number of days in $t$.
| $gc_t = \text{Gross Charges}$
|      The sum of all charges billed during $t$.
| $\overline{dc_t} = \text{Average Daily Charge}$
|      $gc_t$ divided by $n_{days}$.
| $arb_t = \text{AR Balance}$
|      The sum of all outstanding balances at close of business (COB) on the final day of $t$.
| $\overline{dar_t} = \text{Average Days in AR}$
...
And there we have it, our Average Daily Charge is **\$4,240** and our Days in AR is **47.99**. See @nte-example-1.
...
# DAR Targets
   ...
   - $eart = (dart \cdot gc) \div n_{days}$ 
   - $gct = (earb \cdot ndip) \div dart$
