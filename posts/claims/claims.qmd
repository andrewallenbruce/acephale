...
#| message: false
#| warning: false
cclaims |> cheapr::overview(hist = TRUE, digits = 2)
```
...
cclaims |> 
  ...
  count(dos) |> 
  ggplot() + 
  geom_step(aes(dos, n), color = "#E87800", direction = "hv") +
  fuimus::gg_theme()
```
...
cclaims |> 
  ...
  distinct() |> 
  count(dos, pos_type, render) |> 
  mutate(n = cumsum(n), .by = c(pos_type, render)) |> 
  ggplot() + 
  geom_step(aes(dos, n, colour = pos_type), direction = "hv") +
  fuimus::gg_theme()
```
