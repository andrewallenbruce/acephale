...
# Rule Anatomy
   ...
   - Rule Descriptors
      - `rnm`: Unique Alphanumeric Identifier
      - `rid`: Unique Numeric Identifier; Primary Key
         - `rnm == rid`: *Both refer to the same rule*
      - `rdesc`: Single-Line, Human Readable Description of Rule Steps
      - `rmsg`: Rationale for Rule Implementation
      - `rcat`: High-Level Classification of Rule
  - Step Components
      - `rord`: Indicates Order of Step in Rule Validation
      - `rvar`: Variable to Evaluate
      - `ract`: Operation Performed to Test for Expected State
      - `rval`: Value Representing Expected State of Rule Variable


## Rule Descriptors

-   [ ] `rnm`: split letters/numbers, categorize further
-   [X] `rcat`: Extract urls to own column
...
```{r}
rules_desc_summary <- rules_desc |> 
  count_unique(
    rnm, 
    rcat, 
    rdesc, 
    rmsg, 
    .set_names = names(rules_desc[2:5]))

rules_desc_summary |> 
  filter(variable == "rcat")
```
...
rules_desc <- rules_desc |> 
  mutate(https = str_extract_all(rcat,"https?://[^\\s]+"),
         rcat = str_remove_all(rcat,"https?://[^\\s]+")) |>
  unnest(https, keep_empty = TRUE) |>
  separate_longer_delim(https, delim = "https:") |>
  mutate(https = na_if(https, "") |> str_squish(),
         https = if_else(!is.na(https), str_c("https:", https), https),
         rcat = na_if(rcat, "") |> str_squish(),
         rdesc = str_squish(rdesc)) |> 
  nest(urls = c(https))

rules_desc |>
  mutate(rcat = str_wrap(rcat, width = 50),
         rdesc = str_wrap(rdesc, width = 50),
         rmsg = str_wrap(rdesc, width = 50)) |>
  select(-urls) |> 
  gt() |>
  ...
  tab_style(
      ...
      locations = cells_body(columns = c(rdesc))) |>
...
rules_parts |> 
  ...
  tab_style(
      style = cell_text(
        ...
        weight = "bold",
        font = google_font(name = "Roboto Mono")),
      locations = cells_body(columns = c(var_action))) |>
  tab_options(table.width = pct(100),
              quarto.disable_processing = TRUE)
```
...
rules_parts |> 
  ...
  tab_style(
      style = cell_text(
        ...
        weight = "bold",
        font = google_font(name = "Roboto Mono")),
      locations = cells_body(columns = c(rvar, op, rval))) |>
  tab_options(table.width = pct(100),
              quarto.disable_processing = TRUE)
```
...
rules_parts |> 
  ...
  tab_style(
      style = cell_text(
        ...
        weight = "bold",
        font = google_font(name = "Roboto Mono")),
      locations = cells_body(columns = c(rvar, op))) |>
  tab_options(table.width = pct(100),
              quarto.disable_processing = TRUE)
```
...
rules_parts |> 
  ...
  mutate(wc = case_when(str_detect(rval, "\\*") ~ 1L, .default = 0L),
         nchar = if_else(wc ==1, str_count(rval, "[[:alnum:]]"), 0L),
         type = "<chr>",
         .after = rord) |> 
  mutate(op = case_when(
           ract == "is" & wc == 1 ~ "%in%",
