---
title: "Medicare's Physician Fee Schedule"
subtitle: "Taking Apart and Reassembling the MPFS into A Leaner, More User-Friendly Form"
format:
  html:
    other-links:
...
This is an overview of the general format of the [Medicare Physician Fee Schedule (MPFS)](https://www.cms.gov/medicare/payment/fee-schedules/physician), specifically the [PFS Relative Value File](https://www.cms.gov/medicare/payment/fee-schedules/physician/pfs-relative-value-files). 


The "file" is a zip archive and typically contains six files. Updates are released on the first day of each quarter. If required, a correction notice may be issued. The following naming convention is used:
...
dplyr::tibble(
  ...
  contents = c(
    ...
    "OPPS-based capitation-adjusted payment amounts. Carrier-priced codes contain the OPPS-based payment capitation only. Carrier prices cannot exceed this capitation."
  )
) |> 
  gt::gt(
    rowname_col = "n",
...
skimr::yank(rvu_skim, "character") |> 
  dplyr::select(
    var = skim_variable,
    na = n_missing,
    pct_cmp = complete_rate,
    min,
    max,
    unique = n_unique
  ) |>
...
cdbk <- datawizard::data_codebook(
  ...
  dplyr::mutate(
    dplyr::across(
      dplyr::where(is.character), ~dplyr::na_if(., ""))) |> 
  dplyr::filter(!is.na(N)) |>
  tidyr::fill(Name, .direction = "down") |>
  fuimus::remove_quiet() |> 
  dplyr::filter(!Name %in% c("hcpcs", "description"))
...
cdbk |> 
  ...
  # gt::cols_width(Missing ~ gt::px(150)) |>
  gt::tab_options(
    heading.align = "left",
    heading.border.bottom.color = "#4758AB",
    heading.title.font.weight = "bold",
