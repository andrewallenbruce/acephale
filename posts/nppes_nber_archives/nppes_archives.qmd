...
```{r}
#| label: address_functions
#| echo: false
make_purpose <- function(mail, prac) {
  data.table::fcase(
      codex::not_na(mail) & codex::na(prac), "M",
      codex::na(mail) & codex::not_na(prac), "P",
      default = "MP")
}
...
```
   ...
   - Need to consider matches on city, state, zip, with different addresses
...
fax <- address |> 
  ...
  collapse::fcompute(
    ...
    purpose = make_purpose(mail_fax, prac_fax)) |>
...
phone <- address |> 
  ...
  collapse::fcompute(
    npi = npi,
    entity = entity,
    purpose = make_purpose(mail_phone, prac_phone),
    address = prac_address,
    city = prac_city,
    state = prac_state,
